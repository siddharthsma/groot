...
type Store interface {
	SaveResultEvent(context.Context, uuid.UUID, stream.Event) (bool, error)
}
...
type SchemaResolver interface {
	GetLatest(context.Context, string) (schemas.Schema, error)
...
func (e *Emitter) EmitResultEvent(ctx context.Context, req EmitRequest) error {
	if req.DeliveryJob.ResultEventID != nil {
		return nil
	}
	...
	fullName := schemas.FullName(eventType, 1)
	schemaVersion := 1
	if e.schemas != nil {
		...
		schema, validationErr := e.schemas.ValidateEvent(ctx, fullName, req.Connector, stream.SourceKindInternal, payload)
	...
	if e.logger != nil {
		e.logger.Info("result_event_emit_started",
			slog.String("delivery_job_id", req.DeliveryJob.ID.String()),
	...
	linked, err := e.store.SaveResultEvent(ctx, req.DeliveryJob.ID, event)
	if err != nil {
	...
	if e.logger != nil {
		e.logger.Info("result_event_emit_succeeded",
			slog.String("delivery_job_id", req.DeliveryJob.ID.String()),
			slog.String("tenant_id", event.TenantID.String()),
			slog.String("result_event_id", event.EventID.String()),
			slog.String("event_type", event.Type),
		)
	}
	return nil
...
func buildPayload(req EmitRequest) (json.RawMessage, error) {
	payload := map[string]any{
		...
		"subscription_id":  req.Subscription.ID.String(),
		"delivery_job_id":  req.DeliveryJob.ID.String(),
