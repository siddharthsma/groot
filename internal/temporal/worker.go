...
func NewWorker(c client.Client, taskQueue string, deps activities.Dependencies) worker.Worker {
	...
	w.RegisterActivityWithOptions(activitySet.ExecuteConnection, activity.RegisterOptions{Name: activities.ExecuteIntegrationName})
	...
	w.RegisterActivity(activitySet.LoadConnection)
