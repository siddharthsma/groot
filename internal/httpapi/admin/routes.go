...
func RegisterAdminRoutes(mux *http.ServeMux, state *common.State) {
	...
	mux.Handle("GET /admin/connector-instances", h.requireAdmin(state.CommunityEditionRestriction(http.HandlerFunc(h.adminConnectorInstances))))
	mux.Handle("PUT /admin/connector-instances/{id}", h.requireAdmin(state.CommunityEditionRestriction(http.HandlerFunc(h.adminConnectorInstance))))
