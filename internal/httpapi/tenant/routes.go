...
func RegisterTenantRoutes(mux *http.ServeMux, state *common.State) {
	...
	mux.Handle("GET /connector-instances", h.requireTenantAuth(http.HandlerFunc(h.connectorInstances)))
	mux.Handle("POST /connector-instances", http.HandlerFunc(h.connectorInstances))
