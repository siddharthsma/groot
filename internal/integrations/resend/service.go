...
type Store interface {
	EnsureConnection(context.Context, tenant.ID, string, time.Time) error
...
func (s *Service) Enable(ctx context.Context, tenantID tenant.ID) (EnableResult, error) {
	if err := s.store.EnsureConnection(ctx, tenantID, IntegrationName, s.now()); err != nil {
		return EnableResult{}, fmt.Errorf("ensure connection: %w", err)
	}

	route, err := s.store.GetInboundRouteByTenant(ctx, IntegrationName, tenantID)
	switch {
	case err == nil:
	case errors.Is(err, sql.ErrNoRows):
		...
		if s.logger != nil {
			s.logger.Info("inbound_route_created", slog.String("integration_name", IntegrationName), slog.String("route_key", route.RouteKey), slog.String("tenant_id", tenantID.String()))
		}
	default:
...
func (s *Service) HandleWebhook(ctx context.Context, rawBody []byte, headers http.Header) error {
	...
	if err != nil {
		if s.logger != nil {
			...
			s.logger.Info("inbound_route_missing", slog.String("integration_name", IntegrationName))
		}
		if s.metrics != nil {
	...
	route, err := s.store.GetInboundRoute(ctx, IntegrationName, token)
	if err != nil {
		if s.logger != nil {
			s.logger.Info("resend_unroutable", slog.String("token", token))
			s.logger.Info("inbound_route_missing", slog.String("integration_name", IntegrationName), slog.String("route_key", token))
		}
		if s.metrics != nil {
			s.metrics.IncResendUnroutable()
			s.metrics.IncInboundUnroutable(IntegrationName)
	...
	if s.logger != nil {
		s.logger.Info("inbound_route_resolved", slog.String("integration_name", IntegrationName), slog.String("route_key", token), slog.String("tenant_id", route.TenantID.String()))
	}
