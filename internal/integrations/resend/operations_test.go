...
func TestExecuteSendEmail(t *testing.T) {
	...
	client := New(config.ResendConfig{APIKey: "re_test", APIBaseURL: server.URL}, server.Client())
	result, err := client.Execute(context.Background(), OperationSendEmail, nil, json.RawMessage(`{"to":"user@example.com","subject":"Hi","text":"Hello"}`), event.Event{})
	if err != nil {
...
func TestExecuteSendEmailRetryable(t *testing.T) {
	...
	client := New(config.ResendConfig{APIKey: "re_test", APIBaseURL: server.URL}, server.Client())
	_, err := client.Execute(context.Background(), OperationSendEmail, nil, json.RawMessage(`{"to":"user@example.com","subject":"Hi","text":"Hello"}`), event.Event{})
