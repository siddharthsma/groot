...
func New(apiBaseURL, apiVersion string, httpClient HTTPClient) *Client {
...
func (c *Client) Execute(ctx context.Context, operation string, instanceConfig, params json.RawMessage, _ event.Event) (outbound.Result, error) {
	...
	if err := json.Unmarshal(instanceConfig, &cfg); err != nil {
		return outbound.Result{}, outbound.PermanentError{Err: fmt.Errorf("decode connection config: %w", err)}
	}
	if strings.TrimSpace(cfg.IntegrationToken) == "" {
		return outbound.Result{}, outbound.PermanentError{Err: connection.ErrMissingNotionToken}
	}
...
func (c *Client) execute(ctx context.Context, method, url, token string, body []byte, idExtractor func([]byte) (string, error)) (outbound.Result, error) {
