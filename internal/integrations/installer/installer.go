...
import (
	...
	"groot/internal/integrations/registryclient"
)
...
func (s *Service) InstallName(ctx context.Context, name string) (InstallResult, error) {
	...
	if !ok {
		return InstallResult{}, fmt.Errorf("integration %s not found in registry", name)
	}
	...
	return InstallResult{}, fmt.Errorf("integration %s has no installable versions", name)
}
...
func (s *Service) Remove(name string) error {
	...
	if !ok {
		return fmt.Errorf("integration %s is not installed", name)
	}
...
func (s *Service) List() ([]InstalledIntegration, error) {
	installed, err := LoadInstalled(s.cfg.InstalledPath)
	if err != nil {
		return nil, err
	}
	return installed.Integrations, nil
}

func (s *Service) Info(name string) (InstalledIntegration, error) {
	installed, err := LoadInstalled(s.cfg.InstalledPath)
	if err != nil {
		return InstalledIntegration{}, err
	}
	record, ok := LookupInstalled(installed, name)
	if !ok {
		return InstalledIntegration{}, fmt.Errorf("integration %s is not installed", name)
	}
	return record, nil
}
...
func (s *Service) installPackage(ctx context.Context, body []byte, packageFilename string, checksum string) (InstallResult, error) {
	...
	tempDir, err := os.MkdirTemp("", "groot-integration-install-*")
	if err != nil {
	...
	if err := integration.ValidateSpec(loaded.Spec()); err != nil {
		return InstallResult{}, fmt.Errorf("validate integration spec: %w", err)
	}
	if strings.TrimSpace(loaded.Spec().Name) != strings.TrimSpace(contents.Manifest.Name) {
		return InstallResult{}, fmt.Errorf("manifest.name does not match plugin integration name")
	}
	if err := verifyIntegrationSpecHash(contents.Manifest, loaded.Spec()); err != nil {
		return InstallResult{}, err
	}
