...
func TestGenerateOpenAI(t *testing.T) {
	...
	result, err := client.Execute(context.Background(), OperationGenerate,
		json.RawMessage(`{"default_integration":"openai","integrations":{"openai":{"api_key":"env:OPENAI_API_KEY"}}}`),
...
func TestSummarizeAnthropicRetryable(t *testing.T) {
	...
	_, err := client.Execute(context.Background(), OperationSummarize,
		json.RawMessage(`{"default_integration":"anthropic","integrations":{"anthropic":{"api_key":"env:ANTHROPIC_API_KEY"}}}`),
		json.RawMessage(`{"text":"Long text","integration":"anthropic"}`),
		event.Event{},
	)
...
func TestClassifyUsesConfiguredDefaultModel(t *testing.T) {
	...
	result, err := client.Execute(context.Background(), OperationClassify,
		json.RawMessage(`{"default_integration":"openai","integrations":{"openai":{"api_key":"env:OPENAI_API_KEY"}}}`),
...
func TestExtractReturnsStructuredJSON(t *testing.T) {
	...
	result, err := client.Execute(context.Background(), OperationExtract,
		json.RawMessage(`{"default_integration":"openai","integrations":{"openai":{"api_key":"env:OPENAI_API_KEY"}}}`),
