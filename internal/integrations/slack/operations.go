...
func New(apiBaseURL string, httpClient HTTPClient) *Client {
	...
	return &Client{apiBaseURL: strings.TrimRight(apiBaseURL, "/"), httpClient: httpClient}
}
...
func buildRequest(operation string, instanceConfig, params json.RawMessage) (Config, postMessageRequest, error) {
	...
	if err := json.Unmarshal(instanceConfig, &cfg); err != nil {
		return Config{}, postMessageRequest{}, fmt.Errorf("decode connection config: %w", err)
	}
	if strings.TrimSpace(cfg.BotToken) == "" {
		return Config{}, postMessageRequest{}, connection.ErrMissingBotToken
	}
