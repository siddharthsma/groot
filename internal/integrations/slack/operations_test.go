...
func TestExecutePostMessage(t *testing.T) {
	...
	result, err := client.Execute(context.Background(), OperationPostMessage, json.RawMessage(`{"bot_token":"xoxb-test"}`), json.RawMessage(`{"channel":"#alerts","text":"hello"}`), event.Event{Timestamp: time.Now()})
	if err != nil {
...
func TestExecuteReturnsPermanentAuthError(t *testing.T) {
	...
	client := New(server.URL, server.Client())
	_, err := client.Execute(context.Background(), OperationPostMessage, json.RawMessage(`{"bot_token":"xoxb-test"}`), json.RawMessage(`{"channel":"#alerts","text":"hello"}`), event.Event{})
...
func TestExecuteCreateThreadReply(t *testing.T) {
	...
	client := New(server.URL, server.Client())
	result, err := client.Execute(context.Background(), OperationCreateThreadReply, json.RawMessage(`{"bot_token":"xoxb-test"}`), json.RawMessage(`{"channel":"C123","thread_ts":"123.45","text":"reply"}`), event.Event{Timestamp: time.Now()})
	if err != nil {
		t.Fatalf("Execute() error = %v", err)
	}
