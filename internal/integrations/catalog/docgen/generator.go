...
func main() {
	...
	outDir := filepath.Join(root, "docs", "integrations", "generated")
	if err := os.MkdirAll(outDir, 0o755); err != nil {
		panic(err)
	}
	svc := catalog.NewService(nil)
	integrations, err := svc.List(context.Background())
	if err != nil {
		panic(err)
	}
	for _, summary := range integrations {
...
func render(detail catalog.IntegrationDetail) string {
	var b strings.Builder
	b.WriteString("# " + title(detail.Name) + " Integration\n\n")
	b.WriteString("## Integration Name\n\n")
