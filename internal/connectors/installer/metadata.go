...
func LoadInstalled(path string) (InstalledFile, error) {
	...
	sort.Slice(installed.Providers, func(i, j int) bool {
		return installed.Providers[i].Name < installed.Providers[j].Name
	})
...
func UpsertInstalled(installed InstalledFile, record InstalledProvider) InstalledFile {
	next := make([]InstalledProvider, 0, len(installed.Providers)+1)
	replaced := false
	for _, current := range installed.Providers {
	...
	installed.Providers = next
	return installed
}

func RemoveInstalled(installed InstalledFile, name string) InstalledFile {
	filtered := make([]InstalledProvider, 0, len(installed.Providers))
	for _, current := range installed.Providers {
	...
	installed.Providers = filtered
	return installed
}

func LookupInstalled(installed InstalledFile, name string) (InstalledProvider, bool) {
	for _, current := range installed.Providers {
		if current.Name == name {
			return current, true
		}
