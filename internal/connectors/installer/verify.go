...
func verifyManifest(manifest Manifest, currentVersion string) error {
	...
	if strings.TrimSpace(manifest.ProviderSpecHash) == "" {
		return errors.New("manifest.provider_spec_hash is required")
	}
	...
	if manifest.BuildOS != runtime.GOOS || manifest.BuildArch != runtime.GOARCH {
		return fmt.Errorf("provider package targets %s/%s, current platform is %s/%s", manifest.BuildOS, manifest.BuildArch, runtime.GOOS, runtime.GOARCH)
	}
...
func verifyProviderSpecHash(manifest Manifest, spec provider.ProviderSpec) error {
	hash, err := provider.SpecHash(spec)
	if err != nil {
		return fmt.Errorf("hash provider spec: %w", err)
	}
	if hash != strings.TrimSpace(manifest.ProviderSpecHash) {
		return fmt.Errorf("provider_spec_hash mismatch")
	}
	return nil
}
