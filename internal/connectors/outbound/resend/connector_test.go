...
func TestExecuteSendEmail(t *testing.T) {
	...
	result, err := connector.Execute(context.Background(), OperationSendEmail, nil, json.RawMessage(`{"to":"user@example.com","subject":"Hi","text":"Hello"}`), stream.Event{})
	if err != nil {
...
func TestExecuteSendEmailRetryable(t *testing.T) {
	...
	connector := New(config.ResendConfig{APIKey: "re_test", APIBaseURL: server.URL}, server.Client())
	_, err := connector.Execute(context.Background(), OperationSendEmail, nil, json.RawMessage(`{"to":"user@example.com","subject":"Hi","text":"Hello"}`), stream.Event{})
