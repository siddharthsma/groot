...
func TestExecutePostMessage(t *testing.T) {
	...
	result, err := connector.Execute(context.Background(), OperationPostMessage, json.RawMessage(`{"bot_token":"xoxb-test"}`), json.RawMessage(`{"channel":"#alerts","text":"hello"}`), stream.Event{Timestamp: time.Now()})
	if err != nil {
...
func TestExecuteReturnsPermanentAuthError(t *testing.T) {
	...
	connector := New(server.URL, server.Client())
	_, err := connector.Execute(context.Background(), OperationPostMessage, json.RawMessage(`{"bot_token":"xoxb-test"}`), json.RawMessage(`{"channel":"#alerts","text":"hello"}`), stream.Event{})
...
func TestExecuteCreateThreadReply(t *testing.T) {
	...
	connector := New(server.URL, server.Client())
	result, err := connector.Execute(context.Background(), OperationCreateThreadReply, json.RawMessage(`{"bot_token":"xoxb-test"}`), json.RawMessage(`{"channel":"C123","thread_ts":"123.45","text":"reply"}`), stream.Event{Timestamp: time.Now()})
	if err != nil {
		t.Fatalf("Execute() error = %v", err)
	}
