...
func TestCreatePage(t *testing.T) {
	...
	connector := New(server.URL+"/v1", "2022-06-28", server.Client())
	result, err := connector.Execute(context.Background(), OperationCreatePage,
...
func TestAppendBlockRateLimited(t *testing.T) {
	...
	connector := New(server.URL, "2022-06-28", server.Client())
	_, err := connector.Execute(context.Background(), OperationAppendBlock,
		json.RawMessage(`{"integration_token":"secret_xxx"}`),
