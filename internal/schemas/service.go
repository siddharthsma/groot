...
func (s *Service) RegisterBundles(ctx context.Context, bundles []Bundle) error {
	for _, bundle := range bundles {
		for _, spec := range bundle.Schemas {
			...
			if s.logger != nil {
				s.logger.Info("schema_registered",
					slog.String("bundle", bundle.Name),
					slog.String("full_name", record.FullName),
				)
			}
		}
	}
...
func (s *Service) List(ctx context.Context) ([]Schema, error) {
	schemas, err := s.store.ListEventSchemas(ctx)
	if err != nil {
...
func (s *Service) Get(ctx context.Context, fullName string) (Schema, error) {
	schema, err := s.store.GetEventSchema(ctx, strings.TrimSpace(fullName))
	if err != nil {
...
func (s *Service) GetLatest(ctx context.Context, eventType string) (Schema, error) {
	schema, err := s.store.GetLatestEventSchema(ctx, strings.TrimSpace(eventType))
	if err != nil {
...
func (s *Service) ValidateEvent(ctx context.Context, fullName, source, sourceKind string, payload json.RawMessage) (Schema, error) {
	...
	if schema.Source != "" && schema.Source != strings.TrimSpace(source) {
		return schema, s.handleValidationFailure(fullName, fmt.Sprintf("source mismatch: got %s want %s", source, schema.Source))
	}
	if schema.SourceKind != "" && schema.SourceKind != strings.TrimSpace(sourceKind) {
		return schema, s.handleValidationFailure(fullName, fmt.Sprintf("source_kind mismatch: got %s want %s", sourceKind, schema.SourceKind))
	}
	if s.cfg.ValidationMode == ValidationModeOff {
		return schema, nil
	}
	if err := validateJSONSchema(schema.SchemaJSON, payload); err != nil {
		return schema, s.handleValidationFailure(fullName, err.Error())
	}
	return schema, nil
}
...
func (s *Service) ValidateTemplatePaths(ctx context.Context, fullName string, value any) error {
	...
	parsed, err := parseSchema(schema.SchemaJSON)
	if err != nil {
	...
	if err := s.validateTemplates(value, parsed); err != nil {
		...
		if errors.As(err, &templateErr) {
			...
			if s.logger != nil {
				s.logger.Info("subscription_template_invalid",
					slog.String("full_name", fullName),
					slog.String("path", templateErr.Path),
				)
			}
		}
		return err
...
func (s *Service) handleValidationFailure(fullName, reason string) error {
	...
	if s.logger != nil {
		s.logger.Info("schema_validation_failed",
			slog.String("full_name", fullName),
...
func (s *Service) validateTemplates(value any, schema schemaNode) error {
	switch typed := value.(type) {
	case string:
		return validateTemplateString(typed, schema)
	case []any:
		for _, item := range typed {
			if err := s.validateTemplates(item, schema); err != nil {
				return err
			}
		}
	case map[string]any:
	...
	default:
		...
		return s.validateTemplates(decoded, schema)
	}
	return nil
}

func validateTemplateString(text string, schema schemaNode) error {
